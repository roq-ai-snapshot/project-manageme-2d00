...
import { getUserTeams, deleteUserTeamById } from 'apiSdk/user-teams';
import { UserTeamInterface } from 'interfaces/user-team';

type ColumnType = ColumnDef<UserTeamInterface, unknown>;

interface UserTeamListPageProps {
...
export function UserTeamListPage(props: UserTeamListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<UserTeamInterface>>(
    () => `/user-teams?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: UserTeamInterface) => {
    if (hasAccess('user_team', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/user-teams/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/user-teams/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('user_team', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/user-teams/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('user_team', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="user_team" />
        </Flex>

        {hasAccess('user_team', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/user-teams/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}

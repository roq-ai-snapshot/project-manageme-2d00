...
import { getUserTeams } from 'apiSdk/user-teams';
...
function DashboardPage() {
  ...
  const userTeamStat = useSWR<PaginatedInterface<UserTeamInterface>>(
    () => `/user-teams`,
    () => getUserTeams({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'User Teams',
      result: userTeamStat?.data?.totalCount,
      isLoading: userTeamStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },

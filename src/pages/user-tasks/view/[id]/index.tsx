...
import { getUserTaskById } from 'apiSdk/user-tasks';
import { UserTaskInterface } from 'interfaces/user-task';

function UserTaskViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<UserTaskInterface>(
    () => (id ? `/user-tasks/${id}` : null),
    () =>
      getUserTaskById(id, {
        relations: ['user', 'task'],
      }),
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('user_task', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/user-tasks/edit/${id}`} passHref legacyBehavior>
                    <Button
